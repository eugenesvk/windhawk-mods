...
// Bugs/FRs: github.com/ramensoftware/windhawk-mods/issues
...
Based on the [Win11DisableRoundedCorners](https://github.com/valinet/Win11DisableRoundedCorners) project by Valentin Radu.
![Screenshot](https://i.imgur.com/ez0jyuW.png)
## ⚠ Important usage note ⚠
In order to use this mod, you must allow Windhawk to inject into the **dwm.exe** system process. To do so, add it to the process inclusion list in the advanced settings. If you do not do this, it will silently fail to inject.
...
int   WINAPI GetEffectiveCornerStyle_Hook     () {return 0;}

BOOL Wh_ModInit() { Wh_Log(L">");
  HMODULE udwm = GetModuleHandle(L"udwm.dll");
  if (!udwm) {Wh_Log(L"udwm.dll isn't loaded"); return FALSE;}
